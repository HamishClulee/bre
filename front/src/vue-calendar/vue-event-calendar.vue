...
<template>
    <div class="__vev_calendar-wrapper">
        ...
        <cal-events v-if="showEventsOnMobile && !isMobile">
            <slot></slot>
        </cal-events>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...mapGetters(['getCalendar', 'getCourses', 'getWindowWidth']),
        isMobile () {
            return this.getWindowWidth < 640
        }
    },
    mounted () {
        this.GET_ALL_UPCOMING_COURSES()
    },
    methods: {
        ...mapActions(['GET_ALL_UPCOMING_COURSES'])
    }
}
</script>